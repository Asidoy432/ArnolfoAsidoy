...
def init_db():
    ...
    if conn.execute("SELECT COUNT(*) FROM students").fetchone()[0] == 0:
        conn.executemany("INSERT INTO students (name, grade, section) VALUES (?, ?, ?)",
            [("Juan", 85, "Stallman"), ("Maria", 90, "Stallman"), ("Pedro", 70, "Zechariah")])
    conn.commit()
    conn.close()
...
def list_students():
    ...
    <table border="1" cellpadding="8" cellspacing="0">
      <tr><th>ID</th><th>Name</th><th>Grade</th><th>Section</th><th>Remark</th><th>Actions</th></tr>
      {% for s in students %}
      <tr>
        <td>{{ s['id'] }}</td>
        <td>{{ s['name'] }}</td>
...
## File 3 — `Procfile`
⚠️ **No file extension. Filename is exactly:** `Procfile`
```
